...
/**
 * The {@code EclipseStoreProperties} class holds the configuration properties
 * for the Eclipse Store.
 * These properties are loaded from the application's configuration files and
 * can be used to configure the Eclipse Store.
 *
 * <p>
 * Each property in this class corresponds to a configuration option for the
 * Eclipse Store.
 * The properties are loaded from the configuration files when the application
 * starts.
 * </p>
 */
public class EclipseStoreProperties {
    ...
    /**
     * Specify the complete path for the class, which will serve as the root. This
    ...
    /**
     * The base directory of the storage in the file system. Default is "storage" in
    ...
    /**
     * If configured, the storage will not delete files. Instead of deleting a file
     ...
     * Every thread has exclusive access to its directory. Default is 1.
     */
    ...
    /**
     * Interval for the housekeeping. This is work like garbage collection or cache
     * checking.
     * In combination with houseKeepingNanoTimeBudget the maximum processor time for
     * housekeeping work can be set.
     * Default is 1 second.
     */
    ...
    /**
     * Usage of an adaptive housekeeping controller, which will increase the time
     * budgets on demand,
    ...
    /**
     * The threshold of the adaption cycle to calculate new budgets for the
     * housekeeping process. Default is 5 seconds.
     */
    private String housekeepingIncreaseThreshold;

    /**
     * The amount the housekeeping budgets will be increased each cycle. Default is
     * 50 ms.
     */
    ...
    /**
     * The maximum size of a transaction file. If the file is larger than this
     * value, it will be split into multiple files.
     * Default is 1 GiB.
     */
     ...
     * If an entity was not accessed in this timespan it will be removed from the
     * cache. Default is 1 day.
     */
    private String entityCacheTimeout;

    /**
     * Abstract threshold value for the lifetime of entities in the cache. Default
     * is 1000000000.
     */
     ...
     * prevent the file from being dissolved. Default is 0.75 (75%).
     */
    ...
    /**
     * Is the {@code StorageManager} started when the CDI bean for the instance is
     * created or not.
     * Be aware that when you don't rely on the autostart of the StorageManager, you
     * are responsible for starting it
     * and might result in Exceptions when code is executed that relies on a started
     * StorageManager.
    ...
    /**
     * A flag defining whether the default handlers for JDK 1.7 classes shall be
     * registered or not.
     * Default value is true.
     */
     ...
     * By default, this is set to false for compatibility reasons with previously
     * created storage.
     * Enabling this requires adding these handlers to your storage configuration
     * later when used in another application with Eclipse Store.
     ...
     * If no user-defined bean with EmbeddedStorageFoundation exists, a default one
     * will be created.
     ...
     * If no user-defined bean with EmbeddedStorage exists, a default one will be
     * created.
     * To prevent this automatic creation, set this flag to false.
     */
