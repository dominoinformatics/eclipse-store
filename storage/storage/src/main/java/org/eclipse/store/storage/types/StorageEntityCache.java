...
public interface StorageEntityCache<E extends StorageEntity> extends StorageChannelResetablePart {
	...
	public final class Default
		...
		// state 1.0: immutable or stateless (as far as this implementation is
		// concerned)
		...
		// state 2.0: final references to mutable instances, i.e. content must be
		// cleared on reset
		...
		// Statistics for debugging / monitoring / checking to compare with other
		// channels and with the markmonitor
		...
		Default(
			...
			// create reference marker at the very end to have all state properly
		...
		public final synchronized void reset() {
			...
			// create a new root type instance on every clear. Everything else is not worth
			// the reset&register-hassle.
			this.rootType = this.getType(this.rootTypeId);

		}

		private void resetLiveCursor() {
			// live cursor may never be a head dummy-entity (but it may be a tail entity as
			// this is checked)
			this.liveCursor = null;

		}

		// must use lock to keep other channels from marking while rebuild is in
		// progress
		private void enlargeOidHashTable() {
			final int newModulo;
			final StorageEntity.Default[] newSlots = XMath.isGreaterThanOrEqualHighestPowerOf2(this.oidHashTable.length)
		...
		private void rebuildTidHashTable() {
			final int newModulo;
			final StorageEntityType.Default[] newSlots = new StorageEntityType.Default[(newModulo = (this.tidModulo
					+ 1 << 1) - 1) + 1];
		...
		private StorageEntityType.Default addNewType(final long typeId) {
			// the order is important: first rebuild hash table, THEN create and register
			// the instance. DONT MESS UP.
			...
			// explicit hash index for debug purposes. Creating types is not performance
			// critical.
		...
		static final int hashNormalized(final long value, final int bitShiftCount, final int modulo) {
			// (09.08.2015 TM)NOTE: included channel hash mod bit shifting to properly
			// distribute in hash table
		 ...
		 * This method does not need to be synchronized (locked), as it is exclusively
		 * always called by the
		 * channel's inherent thread which is also the same that rebuilds the
		 * hashTables, so it can never work
		...
		final void unregisterEntity(final StorageEntity.Default item) {
			...
			if ((entry = this.getOidHashChainHead(item.objectId())) == item) {
				...
				// subject is (must be) guaranteed to be contained in the hash chain, hence no
				// null check
		...
		private void validateObjectId(final long objectId) {
			...
			if (this.oidChannelIndex(objectId) != this.channelIndex) {
				throw new StorageExceptionConsistency(
						"Invalid objectId " + objectId + " for hash channel " + this.channelIndex);
			}
		}
		...
		final StorageIdAnalysis validateEntities() {
			...
			// validate all entities via iteration by type. Simplifies debugging and
			// requires less type pointer chasing
		...
		final StorageEntity.Default putEntity(final long objectId, final StorageEntityType.Default type) {
			 ...
			 * The normal case (putEntity while storing) is faster if the type has to be
			 * looked up only if it is
			 * really required (creation of a new entry), while the validating case is
			 * faster if the anyway present
			 * type can be linked right away instead of being looked up again.
			 * This is one of the cases, where higher abstraction and redundant code
			 * prevents comes at the price of
		...
		final StorageEntity.Default putEntity(final long entityAddress) {
			 ...
			 * Theoretically, the storage engine (OGS) could also use the switchByteOrder
			 * mechanism implemented for
			 * communication (OGC). However, there are a lot stumbling blocks in the details
			 * that are currently not
			 * worth resolving for a feature that is most probably never required in the
			 * foreseeable future.
			 * This method is one of them. Instead of reading the raw value, additional work
			 * (performance loss)
			 * would have to be done to check if byte reversal is necessary. For now, this
			 * method here alone kills
			 * the byte order switching for storage usage. Should the need arise in the
			 * future, additional
			 ...
			 * loading ~25 million entities took from 32 to 75 seconds and depends
			 * overwhelmingly on
			 * the state of the disc cache and other OS disturbances and hardly on the
			 * actual program code runtime.
			 * 
			 */
			try {
		...
		/**
		 * This purpose of this method is to handle the tricky interaction between data
		 * updates (stores/imports) and GC.
		 * When new data comes in, it has to be ensured that the updated entities'
		 * references are revisited.
		 * See "doomed kept alive" and "slipped through" cases.
		 * This means an entity with references that is already black (saved from sweep
		 * and has its references iterated)
		 * must be demoted to gray (saved from sweep but references not handled yet) and
		 * have its OID enqueued in the gray chain.
		 * <p>
		 * If the entity has no reference, it can be marked black right away. This
		 * either anticipates/replaces the black marking
		 * by the GC and should actually not be necessary, however as the effort to do
		 * it at this point is rather minimal, it's done
		 * nonetheless.
		 */
		private void markEntityForChangedData(final StorageEntity.Default entry) {
			/*
			 * (01.08.2016 TM)NOTE:
			 * Having a sweep pending when data changes requires a distinction here to
			 * achieve correct behavior:
			 * A pending sweep means the marking phase of a gc cycle is complete and all
			 * reachable entities
			 * have already been marked.
			 * The data change did reset the GC completion state.
			 * The following sweep will complete the hot phase.
			 * Enqueuing entities with references here would mean to have them saved in the
			 * next marking phase
			 * (potentially a final/cold gc sweep) even if they are not reachable at all.
			 * This would be an error.
			 * Their references do not have to be checked anyway because the marking has
			 * been completed beforehand
			 * and the purpose of re-marking references is not to safe some priorly
			 * unreachable entity by a race
			 * conditional store. It is only to prevent "slipped through" cases during live
			 * marking.
			 * Entities have, however, to be saved in the coming sweep because they might be
			 * new or referenced
			 * by an entity in the same store and have not been marked in the conventional
			 * way.
			 *
			 * In short:
			 ...
			 * - Mark referential entities gray (actually white would suffice) and enqueue
			 * them into the gray chain.
			 */
			...
			if (entry.hasReferences()) {
				 ...
				 * - Some (random) channels initiates a sweep (all channels get their sweep flag
				 * set)
				 ...
				 * - all channels gray-mark in their entity update the stored entities and
				 * enqueue their OIDs to be marked
				 * - the task processing is completed
				 * - ch #0 and #1 continue doing housekeeping, see the sweep flag and perform
				 * their pending sweep.
				 * - ch #0 and #1 check gray entities in the sweep and (correctly!) rescue them
				 * as they are not white.
				 * - ch #0 and #1 complete their sweep and continue with marking, including
				 * marking the enqueued stored entities
				 *
				 * The gray state might be superfluous if entities are not gray marked if a
				 * sweep is pending,
				 * but this might complicate things and maybe there are other scenarios that
				 * wouldn't be covered
				 * correctly anymore. In the very least, the gray state is a safety net of
				 * indicating:
				 * The entity must not be collected, but it must be revisited.
				 */
				entry.markGray();

				// must mark via mark monitor to keep central mark count consistent. NEVER
				// directly via the queue!
		...
		final void deleteEntity(
			...
			// 2.) detach entity from file registry. Actual physical remains don't hurt,
			// even on restart, as they will be unreachable again.
			entity.detachFromFile();

			// 3.) remove entity from its type registry, effectively removing it from
			// iteration, count and export logic.
		...
		void checkForCacheClear(final StorageEntity.Default entry, final long evalTime) {
			if (this.entityCacheEvaluator.clearEntityCache(this.usedCacheSize, evalTime, entry)) {
				...
				// if the loaded entity data can stay in memory, touch the entity to mark now as
				// its last use.
				entry.touch();
			}
		}
		...
		private void advanceMarking(final int oidsCount) {
			// it is crucial to enqueue cached references effectively before updating the
			// pending marks count.
		...
		/**
		 * Returns {@code true} if there are no more oids to mark and {@code false} if
		 * time ran out.
		...
		private boolean incrementalMark(final long nanoTimeBudgetBound) {
			...
			// index of next oid to be marked (and current amount of already marked oids).
			// Range: [0; oidsMarkAmount]
			int oidsMarkIndex = 0;
			...
			do {
				...
				if (entry == null) {
					// logger.warn("oid: {}",oidsBuffer[oidsMarkIndex - 1]);
					if (!this.zombieOidHandler.handleZombieOid(oidsBuffer[oidsMarkIndex - 1])) {
						// if the handler didn't throw an exception but didn't say it's handled, either,
						// then log it.
						logger.debug("- Storage GC marking encountered zombie ObjectId {} on index {}",
								oidsBuffer[oidsMarkIndex - 1], oidsMarkIndex);
						this.eventLogger.logGarbageCollectorEncounteredZombieObjectId(oidsBuffer[oidsMarkIndex - 1]);
					}
					continue;
				}

				// if the entry is already marked black (was redundantly enqueued), skip it and
				// continue to the next
				if (entry.isGcBlack()) {
					continue;
				}

				// enqueue all reference ids in the mark queue via the central gc monitor
				// instance to account for channel concurrency
				 ...
				 * - iterateReferenceIds already checks for references and returns false if none
				 * are present
				 * - no general touch here to not touch entities without references.
				 */

				// the entry has been fully processed (either has no references or got all its
				// references gray-enqueued), so mark black.
				entry.markBlack();
			} while (System.nanoTime() < nanoTimeBudgetBound);

			// important: if time ran out, the last batch of processed oids has to be
			// accounted for in the gray queue
			if (oidsMarkIndex > 0) {
				// an incremented index always equals an element count
				this.advanceMarking(oidsMarkIndex);
			}
		...
		/**
		 * If an entity (its OID) is still reachable in the application, it may not be
		 * deleted.
		 * Otherwise, data might be lost since the entity still exists for the
		 * application and thus
		 * is only stored by reference but not in full. The passed predicate is a
		 * connection to the
		...
		final void sweep(final _longPredicate isReachableInApplication) {
			...
			for (StorageEntityType.Default sweepType = typeHead; (sweepType = sweepType.next) != typeHead;) {
				...
				for (StorageEntity.Default item, last = sweepType.head; (item = last.typeNext) != null;) {
					// actual sweep: white entities are deleted, non-white entities are marked white
					// but not deleted
			...
			// reset file cleanup cursor to first file in order to ensure the cleanup checks
			// all files for the current state.
			this.fileManager.restartFileCleanupCursor();

			// signal mark monitor that the sweep is complete and provide this channel's
			// valid rootOid
		...
		final static int validateStoragePosition(
			 ...
			 * The solution is to spread a single store's buffers over multiple files if
			 * needed.
			 * However, this is not trivial to do:
			 * A single store must be split into multiple stores,
			 * each store must get its own transactions entry OR a kind of store
			 * continuation entry, etc.
			 * 
			 * This effort is not worth it with the transition of the much more advanced
			 * storage management 2.0
			 * concept in mind. There, store chunks will be spread over multiple store files
			 * anyway and
			 * archive files will be split according to oid range and entity length
			 * automatically.
			 * 
			 * For now, this is simply a technical limitation and one more good reason to
			 * implement the
		...
		final void internalPutEntities(
			...
			// chunk's entities are iterated, put into the cache and have their current
			// storage positions set/updated
		...
		public void postStorePutEntities(
			...
			// reset completion here, too, in case the store happed before the sweep and the
			// post-store happens after it
			this.markMonitor.resetCompletion();
			...
			// must be done by the store task's cleanup, but as it is idempotent, call it
			// here right away
			this.clearPendingStoreUpdate();
		}

		final void clearPendingStoreUpdate() {
			// (21.02.2020 TM)NOTE: this potentially gets called after reset(), so it must
			// be accordingly robust.
		...
		public final StorageEntityType.Default lookupType(final long typeId) {
			for (StorageEntityType.Default typeEntry = this.tidHashTable[tidHashIndex(typeId,
					this.tidModulo)]; typeEntry != null; typeEntry = typeEntry.hashNext) {
		...
		public void copyRoots(final ChunksBuffer dataCollector) {
			 ...
			 * If there may be only one root instance, it should be guaranteed here to
			 * determine the valid
			 * one and ignore the rest.
			 * The tricky part is: this has to be done across all channels and in a thread
			 * safe way.
			 * To achieve this, the MarkMonitor would have to do it in a centralized method.
			 * For that, it has to know the entity caches, which it does / can not
			 * currently.
			 * Also, letting one thread call methods of all channels would mean to break the
			 * strict thread locality
			 * of the channels (only the dedicated channel thread may operate on the
			 * EntityCache instances).
			 * Thread-local work of a channel would suddenly have to subject to a lock on
			 * the mark monitor
			 *
			 * This issue is ignored for now, but must be fixed if root instances are to be
			 * replaceable.
			 *
			 * Clean solution:
			 * Copy all roots, but not directly into a ChunksBuffer, but into a special
		...
		private boolean internalCacheCheck(
			...
			if (this.liveCursor == null || !this.liveCursor.isProper() || this.liveCursor.isDeleted()) {
				// cursor special cases: not set, yet or a head/tail instance or meanwhile
				// deleted (= unreachable)
				cursor = this.fileManager.getFirstEntity();

				// special special case: all files are (effectively) empty. Nothing to check.
			...
			// abort condition is checked at the end to guarantee one entity progress to
			// avoid starvation
			do {
				// if the end of one file is reached, the next file gets checked. The last file
				...
				if (entity == tail) {
					...
					// jumps to loop condition check. The next file's first entry might be the
					// cursor!
					continue;
				}
				...
				if (this.entityRequiresCacheClearing(entity, evaluator, evaluationTime)) {
					...
					// check if this was the last entity in the cache, effectively suspending live
					// check
					if (this.usedCacheSize == 0) {
						break;
					}
			...
			// abort conditions for one housekeeping cycle: cursor is encountered again
			// (full loop) or time is up.
		...
		private boolean quitLiveCheck(final StorageEntity.Default entity) {
			...
			// keep last checked entity as a cursor / base / starting point for the next
			// cycle's check
		 ...
		 * Without this method, the cache check would occasionally run indefinitely in
		 * certain data constellations.
		 * With this method, resulting in IDENTICAL logic (but slightly different byte
		 * code), the
		...
		// CHECKSTYLE.OFF: FinalParameters: this method is just an outsourced
		// scroll-helper
		...
		public final boolean issuedGarbageCollection(
			...
			performGC: while (System.nanoTime() < nanoTimeBudgetBound) {
				// call gc for the given time budget and evaluate result
				if (!this.incrementalGarbageCollection(nanoTimeBudgetBound, channel)) {
					// if the call returned indicating that time ran out, return accordingly
					// immediately.
					return false;
				}
				// reaching here means the gc loop ran out of work before it ran out of time.
				// Check why.

				/*
				 * This is a tricky wait loop:
				 * The loop itself only checks for completion and time budget.
				 * Inside the loop, a simple if checks for newly enqueued oids to be
				 * marked/processed (= new work)
				 ...
				 */
				waitForWork: while (System.nanoTime() < nanoTimeBudgetBound) {
					...
					// better try for pending local mark oids to flush before checking/waiting for
					// work
					this.referenceMarker.tryFlush();

					// check/wait for missing oids to mark, which have to be provided by other
					// channels' marking.
					synchronized (this.oidMarkQueue) {
						...
						try {
							...
							// thread has been interrupted while trying to perform garbage collection. So
							// abort and return.
							break performGC;
						}
					}
			...
			// either time ran out or thread was interrupted. In any case, report back the
			// current state of the garbage collection.
		...
		private boolean checkForGcCompletion() {
			if (this.markMonitor.isComplete(this)) {
				// minimize hash table memory consumption if storage is potentially going to be
				// inactive
		...
		private final boolean internalIncrementalGarbageCollection(
			...
			if (this.markMonitor.needsSweep(this)) {
				...
				// must check for completion again, otherwise a channel might restart marking
				// beyond a completed gc.
				if (this.checkForGcCompletion()) {
					return true;
				}

				// check if there is still time to proceed with the next (second) marking right
				// away
			...
			if (this.incrementalMark(nanoTimeBudgetBound)) {
				/*
				 * note:
				 * if the markingOidBuffer length is too low, this return is done countless
				 * times per millisecond.
				 * Initially, 500 was chosen, but 10000 or 50000 seem to be much more
				 * appropriate.
				 */
